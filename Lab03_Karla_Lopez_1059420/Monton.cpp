...
void Monton::agregar_carta(int num2, char color2) {
...
void Monton::quitar_carta(int posicion) {
	Carta* quitar = carta1;
	if (posicion == 0)
	{
		carta1 = nullptr;
	...
	else
	{
		if (posicion < cantidad)
		{
			...
			cantidad = contador;
			quitar->siguiente = quitar->siguiente->siguiente;
			quitar->siguiente->siguiente = nullptr;
		}
	}
}
Carta Monton::obtener_carta(int posicion1) {
	Carta* dato = carta1;
	if (posicion1 == 0)
	{
		return *carta1;
